...
fn local_cpu_compose_bundle_matches_spec_artifact_shape() {
    ...
    assert_shell_logical_lines(
        ...
        &[
            ...
            r#"if [ "${LIVE_PENDING_CHALLENGE_REWARD_COUNT:-0}" -gt 0 ] && [ "${LIVE_DELAYED_CHALLENGE_REWARD_CLAIMS:-0}" -le 0 ]; then"#,
            r#"fail "live challenge rewards were present but did not expose future-maturity pending challenge reward claims""#,
            r#"if [ "$LIVE_ROLE_NETWORK_BLOCK_CHECK_CHALLENGES_APPLIED" -gt 0 ] && [ "${LIVE_DELAYED_CHALLENGE_REWARD_CLAIMS:-0}" -le 0 ]; then"#,
            r#"fail "applied live block-check challenges did not expose future-maturity pending challenge reward claims""#,
    ...
    assert_shell_logical_lines(
        ...
        &[
            ...
            r#"live_block_check_challenge_reward_evidence=$([ "$LIVE_ROLE_NETWORK_BLOCK_CHECK_CHALLENGES_APPLIED" -gt 0 ] && printf '%s' true || printf '%s' false)"#,
