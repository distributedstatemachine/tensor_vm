...
fn local_cpu_compose_bundle_matches_spec_artifact_shape() {
    ...
    let competitor_validator = compose_service_section(&compose, "validator-01");
    assert_eq!(
        compose_env_value(
    ...
    for service in validators.iter().skip(3) {
        ...
        assert!(
            section.lines().all(|line| !line
                .trim()
                .starts_with("TENSORVM_LOCAL_CPU_SYNTHETIC_JOB_PRODUCER:")),
            "only validator-00 should enable synthetic job publication"
        );
        assert!(
            section.lines().all(|line| {
                let trimmed = line.trim();
                !trimmed.starts_with("TENSORVM_LOCAL_CPU_VALIDATOR_BLOCK_PROPOSER:")
                    || trimmed == r#"TENSORVM_LOCAL_CPU_VALIDATOR_BLOCK_PROPOSER: "true""#
            }),
            "all validators should enable validator block proposal"
        );
    }
    ...
    assert_eq!(
        env_file_value(&env_file, "TENSORVM_LOCAL_CPU_SYNTHETIC_JOB_PRODUCER"),
        "false"
    );
    assert_eq!(
        env_file_value(&env_file, "TENSORVM_LOCAL_CPU_VALIDATOR_BLOCK_PROPOSER"),
        "false"
    );
    assert_eq!(
        env_file_u64(
    ...
    assert_shell_logical_lines(
        ...
        &[
            ...
            r#"LOCAL_CPU_HTTP_TIMEOUT_SECONDS=45"#,
    ...
    assert_shell_logical_lines(
        ...
        &[
            ...
            r#"json_summary_number() {"#,
            r#"json_future_pending_reward_count() {"#,
    ...
    assert_lacks_shell_logical_lines(
        ...
        &[
            ...
            r#"LIVE_RECEIPTS=$(curl -fsS --max-time 45 -H "Authorization: Bearer ${AUTH_TOKEN}" "http://127.0.0.1:${RPC_PORT}/explorer/receipts/latest/500")"#,
            r#"curl -fsS --max-time 45 -H "Authorization: Bearer ${AUTH_TOKEN}" "http://127.0.0.1:${RPC_PORT}${path}" >/dev/null || fail "gateway route is not reachable: $path""#,
            r#"EXPLORER_HEALTH=$(curl -fsS --max-time 45 "http://127.0.0.1:${EXPLORER_PORT}/health")"#,
            r#"EXPLORER_PAGE=$(curl -fsS --max-time 45 "http://127.0.0.1:${EXPLORER_PORT}/")"#,
            r#"LIVE_CHAIN_HEAD=$(curl -fsS --max-time 45 -H "Authorization: Bearer ${AUTH_TOKEN}" "http://127.0.0.1:${RPC_PORT}/chain/head")"#,
            r#"LIVE_OVERVIEW=$(curl -fsS --max-time 45 -H "Authorization: Bearer ${AUTH_TOKEN}" "http://127.0.0.1:${RPC_PORT}/explorer/overview")"#,
            r#"LIVE_RECEIPTS=$(curl -fsS --max-time 45 -H "Authorization: Bearer ${AUTH_TOKEN}" "http://127.0.0.1:${RPC_PORT}/explorer/receipts/latest/${EXPECTED_LIVE_RECEIPT_QUERY_LIMIT}")"#,
            r#"LIVE_TENSOR=$(curl -fsS --max-time 45 -H "Authorization: Bearer ${AUTH_TOKEN}" "http://127.0.0.1:${RPC_PORT}/tensor/latest")"#,
            r#"LIVE_TENSOR_DESCRIPTOR=$(curl -fsS --max-time 45 -H "Authorization: Bearer ${AUTH_TOKEN}" "http://127.0.0.1:${RPC_PORT}/tensor/${LIVE_TENSOR_ID}/descriptor")"#,
            r#"LIVE_TENSOR_ROW=$(curl -fsS --max-time 45 -H "Authorization: Bearer ${AUTH_TOKEN}" "http://127.0.0.1:${RPC_PORT}/tensor/${LIVE_TENSOR_ID}/row/0")"#,
            r#"LIVE_TENSOR_CHUNK=$(curl -fsS --max-time 45 -H "Authorization: Bearer ${AUTH_TOKEN}" "http://127.0.0.1:${RPC_PORT}/tensor/${LIVE_TENSOR_ID}/chunk/0")"#,
            r#"LIVE_TENSOR_OPENING=$(curl -fsS --max-time 45 -H "Authorization: Bearer ${AUTH_TOKEN}" "http://127.0.0.1:${RPC_PORT}/tensor/${LIVE_TENSOR_ID}/opening/0")"#,
            concat!(
            ...
            r#"compose exec -T "$service" grep -q "^local_cpu_synthetic_job_producer=" /var/lib/tensorvm/local-cpu-ready || fail "$service readiness file does not report local CPU synthetic job producer mode""#,
            r#"compose exec -T "$service" grep -q "^local_cpu_validator_block_proposer=" /var/lib/tensorvm/local-cpu-ready || fail "$service readiness file does not report local CPU validator block proposer mode""#,
            r#"compose exec -T "$service" grep -q "^local_cpu_validator_block_proposer_delay_blocks=" /var/lib/tensorvm/local-cpu-ready || fail "$service readiness file does not report local CPU validator block proposer delay""#,
    ...
    assert_shell_logical_lines(
        ...
        &[
            ...
            r#"LIVE_SETTLED_RECEIPT_COUNT=$(json_summary_number settled_receipt_count "$LIVE_OVERVIEW")"#,
            r#"LIVE_FINALIZED_BLOCK_COUNT=$(json_summary_number finalized_block_count "$LIVE_OVERVIEW")"#,
            r#"LIVE_MODEL_STEP_TOTAL=$(json_summary_number model_step_total "$LIVE_OVERVIEW")"#,
            r#"LIVE_PENDING_RECEIPT_REWARD_COUNT=$(json_summary_number pending_receipt_reward_count "$LIVE_OVERVIEW")"#,
            r#"LIVE_PENDING_PROPOSER_REWARD_COUNT=$(json_summary_number pending_proposer_reward_count "$LIVE_OVERVIEW")"#,
            r#"LIVE_PENDING_CHALLENGE_REWARD_COUNT=$(json_summary_number pending_challenge_reward_count "$LIVE_OVERVIEW")"#,
            ...
            r#"[ "${LIVE_FINALIZED_BLOCK_COUNT:-0}" -gt "$EXPECTED_SEED_BLOCKS" ] || fail "gateway finalized block count did not advance past seeded $EXPECTED_SEED_BLOCKS blocks""#,
            r#"[ "${LIVE_JOB_COUNT:-0}" -gt "$EXPECTED_SEED_HEIGHT" ] || fail "protocol did not generate synthetic jobs after seed""#,
            r#"[ "${LIVE_MODEL_COUNT:-0}" -gt 0 ] || fail "protocol did not register a LinearTrainingStep model""#,
            r#"[ "${LIVE_MODEL_STEP_TOTAL:-0}" -gt 0 ] || fail "protocol did not apply a LinearTrainingStep model transition""#,
            ...
            r#"[ "${LIVE_DELAYED_RECEIPT_REWARD_CLAIMS:-0}" -gt 0 ] || fail "live synthetic jobs did not expose future-maturity pending receipt reward claims""#,
            ...
            r#"live_model_step_total=${LIVE_MODEL_STEP_TOTAL}"#,
            ...
            r#"live_delayed_receipt_reward_claims=${LIVE_DELAYED_RECEIPT_REWARD_CLAIMS}"#,
            r#"live_delayed_proposer_reward_claims=${LIVE_DELAYED_PROPOSER_REWARD_CLAIMS}"#,
            r#"live_pending_challenge_rewards=${LIVE_PENDING_CHALLENGE_REWARD_COUNT}"#,
            r#"live_delayed_challenge_reward_claims=${LIVE_DELAYED_CHALLENGE_REWARD_CLAIMS}"#,
        ],
    );
    assert!(
    ...
    assert_status_value_reads(
        ...
        &[
            ...
            (
                "SERVICE_ROLE_LOCAL_BLOCK_PROPOSER_DELAY_BLOCKS",
                "role_local_block_proposer_delay_blocks",
            ),
            (
                "SERVICE_ROLE_LOCAL_BLOCK_PROPOSER_DELAY_SATISFIED",
    ...
    assert_shell_logical_lines(
        ...
        &[
            ...
            concat!(
                ...
                r#"|| [ "$SERVICE_ATTESTATION_COUNT" -le "$SEED_ATTESTATION_COUNT" ] "#,
                r#"|| [ "$SERVICE_ROLE_P2P_CONNECTED_PEERS" -le 0 ] "#,
                r#"|| { ! restart_plateau_service_ready "#,
                r#"&& { [ "$SERVICE_ROLE_P2P_OBSERVED_BLOCK_VOTES" -le 0 ] "#,
                r#"|| [ "$SERVICE_ROLE_P2P_OBSERVED_JOBS" -le 0 ] "#,
                r#"|| [ "$SERVICE_ROLE_P2P_OBSERVED_RECEIPTS" -le 0 ] "#,
            ...
            concat!(
                ...
                r#"|| [ "$SERVICE_LATEST_BLOCK_HEIGHT" -lt "$EXPECTED_SEED_HEIGHT" ] "#,
                r#"|| [ "$SERVICE_LATEST_BLOCK_HASH" = "$ZERO_HASH" ] "#,
                r#"|| [ "$SERVICE_STATE_ROOT" = "$ZERO_HASH" ] "#,
                r#"|| [ "$SERVICE_BLOCK_LOG_ROOT" = "$ZERO_HASH" ] "#,
                r#"|| [ "$SERVICE_ROLE_LATEST_HEIGHT" -le "$EXPECTED_SEED_HEIGHT" ]; then"#
            ),
            r#"if [ "$SERVICE_ROLE_LOCAL_PRODUCER" = "true" ]; then"#,
            r#"LIVE_LOCAL_SYNTHETIC_JOB_PRODUCER_COUNT=$((LIVE_LOCAL_SYNTHETIC_JOB_PRODUCER_COUNT + 1))"#,
            r#"[ "$SERVICE_ROLE_P2P_OBSERVED_BLOCKS" -gt 0 ] || { STATUS_MISMATCH=true; continue; }"#,
            r#"[ "$SERVICE_ROLE_P2P_OBSERVED_BLOCK_PAYLOADS" -gt 0 ] || { STATUS_MISMATCH=true; continue; }"#,
            r#"[ "$SERVICE_ROLE_P2P_LATEST_OBSERVED_BLOCK_HEIGHT" -ge "$EXPECTED_SEED_HEIGHT" ] || { STATUS_MISMATCH=true; continue; }"#,
            r#"[ "$SERVICE_ROLE_P2P_LATEST_OBSERVED_BLOCK_PAYLOAD_HEIGHT" -ge "$EXPECTED_SEED_HEIGHT" ] || { STATUS_MISMATCH=true; continue; }"#,
            ...
            r#"multi_validator_proposer_competition=true"#,
    ...
    assert_shell_logical_lines(
        ...
        &[
            ...
            r#"output=$(printf '%s\n' "$output" | tr -d '\r')"#,
            r#"role_loop_ready=$(status_value role_loop_ready "$output")"#,
            r#"if [ -n "$role_loop_ready" ] && [ "$role_loop_ready" != "unknown" ]; then"#,
            ...
            r#"BLOCK_SCAN_END=$((LIVE_FINALIZED_BLOCK_COUNT - 1))"#,
            r#"[ "$BLOCK_SCAN_END" -le "$LIVE_HEIGHT" ] || BLOCK_SCAN_END="$LIVE_HEIGHT""#,
            r#"BLOCK_SCAN_START=$((BLOCK_SCAN_END - EXPECTED_BLOCK_SCAN_DEPTH))"#,
            ...
            r#"[ "$LIVE_TENSOR_OP_BLOCK_HEIGHT" -ge 0 ] || fail "service block view did not expose finalized live TensorOp receipt evidence""#,
            ...
            r#"RESTART_CONTINUITY_MODE="${TENSORVM_LOCAL_CPU_RESTART_CONTINUITY_MODE:-false}""#,
            r#"RESTART_CONTINUITY_SERVICES="${TENSORVM_LOCAL_CPU_RESTART_SERVICES:-}""#,
            r#"restart_plateau_service_ready() {"#,
            concat!(
                r#"[ "$RESTART_CONTINUITY_MODE" = "true" ] "#,
                r#"&& word_list_contains "$RESTART_CONTINUITY_SERVICES" "$service" "#,
                r#"&& [ "$SERVICE_HEIGHT" -gt "$EXPECTED_SEED_HEIGHT" ] "#,
                r#"&& [ "$SERVICE_BLOCK_COUNT" -gt "$EXPECTED_SEED_BLOCKS" ] "#,
                r#"&& [ "$SERVICE_FINALIZED_BLOCK_COUNT" -gt "$EXPECTED_SEED_BLOCKS" ] "#,
                r#"&& [ "$SERVICE_STATE_ROOT" != "$ZERO_HASH" ] "#,
                r#"&& [ "$SERVICE_BLOCK_LOG_ROOT" != "$ZERO_HASH" ] "#,
                r#"&& [ "$SERVICE_ROLE_P2P_CONNECTED_PEERS" -gt 0 ]"#
            ),
            r#"if restart_plateau_service_ready; then"#,
            r#"elif [ "$SERVICE_ROLE_LOCAL_BLOCK_PROPOSER" = "false" ]; then"#,
            r#"if ! restart_plateau_service_ready; then"#,
            r#"elif restart_plateau_service_ready; then"#,
            ...
            r#"NETWORK_TARGET_HEIGHT=$((CANDIDATE_FINALIZED_BLOCK_COUNT - 1))"#,
            r#"if NETWORK_BLOCK_RAW=$(read_service_block "$EXPECTED_NETWORK_OBSERVER_SERVICE" "$NETWORK_TARGET_HEIGHT"); then"#,
            ...
            r#"CONVERGED_OPERATOR_COUNT=$((CONVERGED_OPERATOR_COUNT + 1))"#,
            r#"EXPECTED_CONVERGED_OPERATOR_COUNT="$EXPECTED_SERVICE_COUNT""#,
            r#"[ "$CONVERGED_OPERATOR_COUNT" = "$EXPECTED_CONVERGED_OPERATOR_COUNT" ] || fail "not all operators satisfied local CPU role, gossip, and delayed proposer evidence""#,
            ...
            r#"live_validator_proposer_networking=true"#,
            r#"live_block_check_challenge_evidence=$([ "$LIVE_ROLE_NETWORK_BLOCK_CHECK_CHALLENGES_APPLIED" -gt 0 ] && printf '%s' true || printf '%s' false)"#,
    ...
    assert_shell_logical_lines(
        ...
        &[
            ...
            r#"timeout "${EXPECTED_DOCKER_EXEC_TIMEOUT_SECONDS}s" docker compose -f "$COMPOSE_FILE" exec -T "$service" curl -fsS --max-time "$EXPECTED_HTTP_TIMEOUT_SECONDS" -H "Authorization: Bearer ${AUTH_TOKEN}" "http://127.0.0.1:8545${path}" 2>/dev/null < /dev/null"#,
            r#"capture_service_tensor before "$service" "$TMP_DIR/before/${service}.tensor""#,
            r#"assert_service_tensor_artifact "$service" "$before_tensor_id" "$before_tensor_root""#,
            r#"sleep "$EXPECTED_CHECKER_RETRY_SLEEP_SECONDS""#,
            r#"finalized_block_count=$(status_value finalized_block_count "$STATUS")"#,
            r#"finalized_head_height=$((finalized_block_count - 1))"#,
            r#"printf 'finalized_head_height=%s\n' "$finalized_head_height""#,
            r#"[ "$latest_block_height" -ge "$EXPECTED_SEED_HEIGHT" ] || fail "$phase status for $service latest block did not reach seeded height $EXPECTED_SEED_HEIGHT""#,
            r#"[ "$min_height" -ge "$EXPECTED_SEED_HEIGHT" ] || fail "$phase common head height did not reach seeded height $EXPECTED_SEED_HEIGHT""#,
            ...
            concat!(
                r#"TENSORVM_LOCAL_CPU_RESTART_CONTINUITY_MODE=true "#,
                r#"TENSORVM_LOCAL_CPU_RESTART_SERVICES="$RESTART_SERVICES" "#,
                r#"timeout "${EXPECTED_RESTART_CHECK_SCRIPT_TIMEOUT_SECONDS}s" "$CHECK_SCRIPT""#
            ),
            r#"ADVANCED_AFTER_RESTART=false"#,
            ...
            r#"restart_heights_advance=${ADVANCED_AFTER_RESTART}"#,
            r#"restart_block_counts_non_decreasing=true"#,
            r#"restart_block_counts_advance=${ADVANCED_AFTER_RESTART}"#,
            r#"restart_state_roots_observed=true"#,
            r#"restart_state_roots_advance=${ADVANCED_AFTER_RESTART}"#,
            r#"restart_block_log_roots_observed=true"#,
            r#"restart_block_log_roots_advance=${ADVANCED_AFTER_RESTART}"#,
            r#"restart_previous_common_head_preserved=true"#,
            r#"restart_previous_common_state_root_preserved=true"#,
            r#"restart_tensor_artifact_preserved=true"#,
            r#"restart_blocks_continue=${ADVANCED_AFTER_RESTART}"#,
    ...
    assert_shell_logical_lines(
        ...
        &[
            ...
            r#"if output=$("$RESTART_SCRIPT" "$service"); then"#,
            r#"printf '%s\n' "$output""#,
            ...
            r#"rolling_restart_heights_advance=${ROLLING_HEIGHTS_ADVANCE}"#,
            r#"rolling_restart_block_counts_advance=${ROLLING_BLOCK_COUNTS_ADVANCE}"#,
            r#"rolling_restart_state_roots_advance=${ROLLING_STATE_ROOTS_ADVANCE}"#,
            r#"rolling_restart_block_log_roots_advance=${ROLLING_BLOCK_LOG_ROOTS_ADVANCE}"#,
            r#"rolling_restart_previous_common_head_preserved=true"#,
            r#"rolling_restart_previous_common_state_root_preserved=true"#,
            r#"rolling_restart_blocks_continue=${ROLLING_BLOCKS_CONTINUE}"#,
