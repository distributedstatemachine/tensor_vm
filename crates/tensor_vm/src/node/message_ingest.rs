use super::{
    ...
    payload_application::{
        ...
        apply_network_receipt_payload, apply_network_trace_bisection_expectation_payload,
        apply_network_trace_bisection_open_payload, apply_network_trace_bisection_referee_payload,
    ...
    for message in network_ingest_order(messages) {
        ...
        match message {
            ...
            P2pMessage::NewTraceBisectionRoundPayload {
                ...
                ingested.block_announcements = ingested.block_announcements.saturating_add(1);
                ingested.trace_bisection_rounds = ingested.trace_bisection_rounds.saturating_add(1);
                if receipt_id == [0; 32]
                    || trace_root == [0; 32]
                    || challenger == [0; 32]
                    || responder == [0; 32]
                ...
                match apply_network_trace_bisection_round_payload(
                    context.chain(),
                    receipt_id,
                    trace_root,
                    challenger,
                    ...
                    NetworkPayloadApply::Pending => {
                        pending_payloads.queue_trace_bisection_round(
                            receipt_id,
                            trace_root,
                            challenger,
                            responder,
            ...
            P2pMessage::NewTraceBisectionExpectationPayload {
                ...
                ingested.block_announcements = ingested.block_announcements.saturating_add(1);
                ingested.trace_bisection_expectations =
                    ingested.trace_bisection_expectations.saturating_add(1);
                if receipt_id == [0; 32]
                    || trace_root == [0; 32]
                    || challenger == [0; 32]
                    || responder == [0; 32]
                ...
                match apply_network_trace_bisection_expectation_payload(
                    context.chain(),
                    receipt_id,
                    trace_root,
                    challenger,
                    ...
                    NetworkPayloadApply::Applied => {
                        ingested.trace_bisection_expectations_applied = ingested
                            .trace_bisection_expectations_applied
                            .saturating_add(1);
                    }
                    NetworkPayloadApply::Pending => {
                        pending_payloads.queue_trace_bisection_expectation(
                            receipt_id,
                            trace_root,
                            challenger,
                            responder,
            ...
            P2pMessage::NewTraceBisectionRefereePayload {
                ...
                ingested.block_announcements = ingested.block_announcements.saturating_add(1);
                ingested.trace_bisection_referees =
                    ingested.trace_bisection_referees.saturating_add(1);
                if challenge_id == [0; 32]
                    || receipt_id == [0; 32]
                    || trace_root == [0; 32]
                    || challenger == [0; 32]
                ...
                match apply_network_trace_bisection_referee_payload(
                    context.chain(),
                    challenge_id,
                    receipt_id,
                    trace_root,
                    ...
                    NetworkPayloadApply::Pending => {
                        pending_payloads.queue_trace_bisection_referee(
                            challenge_id,
                            receipt_id,
                            trace_root,
                            challenger,
...
mod tests {
    ...
    fn trace_bisection_round_context() -> (
        TestNetworkEventContext,
        TraceBisectionConfig,
        TraceBisectionRound,
        Vec<u8>,
    ) {
        let beacon = hash_bytes(b"test", &[b"network-ingest-trace-bisection"]);
        let mut chain = Chain::new(beacon);
        let miner = address(b"network-ingest-trace-miner");
        let challenger = address(b"network-ingest-trace-challenger");
        chain.register_miner(miner, 100).unwrap();
        ...
        chain
            .apply_command(ChainCommand::SubmitReceipt(
                crate::chain::ReceiptState::GraphExecution(receipt.clone()),
            ))
            .unwrap();
        ...
        let expected_output_roots = vec![hash_bytes(b"test", &[b"network-referee-wrong-root"])];
    ...
    fn trace_bisection_referee_context() -> (
        ...
        let beacon = hash_bytes(b"test", &[b"network-ingest-trace-referee"]);
        let mut chain = Chain::new(beacon);
    ...
    #[test]
    fn network_event_driver_applies_and_retries_trace_bisection_expectation_payloads() {
        let (mut context, config, round, _round_payload) = trace_bisection_round_context();
        let mut opened = context.chain.clone();
        opened
            .apply_command(ChainCommand::OpenTraceBisection(config.clone()))
            .unwrap();
        let state = opened
        ...
        let ingested = ingest_network_messages(
            &mut context,
            vec![P2pMessage::NewTraceBisectionExpectationPayload {
        ...
        assert_eq!(ingested.trace_bisection_expectations, 1);
        assert_eq!(ingested.trace_bisection_expectations_applied, 0);
        assert_eq!(ingested.invalid_events, 0);
        assert_eq!(pending.pending_trace_bisection_expectation_count(), 1);
        ...
        assert_eq!(retried.trace_bisection_expectations_applied, 1);
        assert_eq!(retried.invalid_events, 0);
        assert!(pending.is_empty());
        let record = context
            .chain
        ...
        .unwrap();
        assert_eq!(duplicate.trace_bisection_expectations, 1);
        assert_eq!(duplicate.trace_bisection_expectations_applied, 1);
        assert_eq!(duplicate.invalid_events, 0);
        assert!(pending.is_empty());
    }

    #[test]
    fn network_event_driver_applies_and_retries_trace_bisection_round_payloads() {
        let (mut context, config, round, payload) = trace_bisection_round_context();
        let transcript_leaf = round.transcript_leaf();
        let mut pending = PendingNetworkPayloads::default();

        let ingested = ingest_network_messages(
            &mut context,
            vec![P2pMessage::NewTraceBisectionRoundPayload {
        ...
        assert_eq!(ingested.trace_bisection_rounds, 1);
        assert_eq!(ingested.trace_bisection_rounds_applied, 0);
        assert_eq!(ingested.invalid_events, 0);
        ...
        assert_eq!(retried.trace_bisection_rounds_applied, 1);
        assert_eq!(retried.invalid_events, 0);
        assert!(pending.is_empty());
        let record = context
            .chain
            ...
            .values()
            .next()
            .expect("trace bisection session should still exist");
        ...
        .unwrap();
        assert_eq!(duplicate.trace_bisection_rounds, 1);
        assert_eq!(duplicate.trace_bisection_rounds_applied, 1);
        assert_eq!(duplicate.invalid_events, 0);
        assert!(pending.is_empty());
    }

    #[test]
    fn network_event_driver_applies_and_retries_trace_bisection_referee_payloads() {
        let (mut context, config, round, witness, payload) = trace_bisection_referee_context();
        let challenge_id = crate::challenge::trace_bisection_challenge_id(
            &round.receipt_id,
            &round.trace_root,
            &round.challenger,
            &round.responder,
        ...
        let ingested = ingest_network_messages(
            &mut context,
            vec![P2pMessage::NewTraceBisectionRefereePayload {
                challenge_id,
                receipt_id: round.receipt_id,
                trace_root: round.trace_root,
                challenger: round.challenger,
        ...
        assert_eq!(ingested.trace_bisection_referees, 1);
        assert_eq!(ingested.trace_bisection_referees_applied, 0);
        assert_eq!(ingested.invalid_events, 0);
        ...
        assert_eq!(retried.trace_bisection_referees_applied, 1);
        assert_eq!(retried.invalid_events, 0);
        assert!(pending.is_empty());
        let record = context
            .chain
