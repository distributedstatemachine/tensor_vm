...
REQUIRE_COMMITTEE_SETTLEMENT="${TENSORVM_LOCAL_CPU_REQUIRE_COMMITTEE_SETTLEMENT:-true}"
if [ "$REQUIRE_COMMITTEE_SETTLEMENT" = "true" ]; then
  COMMITTEE_SETTLEMENT_FLOOR=1
else
...
while [ "$attempt" -lt "$EXPECTED_CHECKER_RETRY_LIMIT" ]; do
  ...
  if [ "${LIVE_HEIGHT:-0}" -gt "$EXPECTED_SEED_HEIGHT" ] \
    ...
    && [ "${LIVE_SETTLED_COMMITTEE_RECEIPT_COUNT:-0}" -ge "$COMMITTEE_SETTLEMENT_FLOOR" ] \
