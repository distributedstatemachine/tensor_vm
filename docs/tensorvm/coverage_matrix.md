...
`abs`/`sign`/`relu`, mixed-scale fixed-point `add`/`sub` with RHS-to-lhs/output half-even rescale, mixed-scale fixed-point `mul` with half-even product rescale, fixed-point scale-aware half-even `round`, `reshape`, `broadcast`, single-output
...
scale-aware fixed-point `cast`/`round`, mixed-scale `add`/`sub`, and mixed-scale `mul` vectors using per-input and expected output dtype/scale metadata,
...
Remaining Tensor IR/conformance gaps: fixed-point reciprocal division, matmul accumulation/range policy,
