...
same-shape field `mul`, same-shape field comparison masks (`eq`, `gt`, `lt`, `ge`, `le`), and same-shape
field `where` selection from an `Int32` mask in the direct CUDA kernel tests, supported multi-op CPU/GPU
...
- Optional native CUDA kernel support exists behind `--features cuda-kernels` and covers field matmul,
  ...
  `add`/`sub`/`mul`/`relu`/`identity`/`neg`/`abs`/`sign`/`eq`/`gt`/`lt`/`ge`/`le`/`where`/`scalar_mul`/`transpose`, plus linear-step
