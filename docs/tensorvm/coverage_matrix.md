...
deterministic field `sum`/`mean`, field `reshape`/`broadcast`, same-shape field comparison masks
...
- Optional native CUDA kernel support exists behind `--features cuda-kernels` and covers field matmul,
  ...
  `add`/`sub`/`mul`/`div`/`clamp`/`sum`/`mean`/`reshape`/`broadcast`/`relu`/`identity`/`neg`/`abs`/`sign`/`eq`/`gt`/`lt`/`ge`/`le`/`where`/`scalar_mul`/`transpose`, plus linear-step
