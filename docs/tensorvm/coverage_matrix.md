...
`abs`/`sign`/`relu`, mixed-scale fixed-point `add`/`sub` with RHS-to-lhs/output half-even rescale, mixed-scale fixed-point `mul` with half-even product rescale, fixed-point reciprocal `div` with lhs/output scale half-even quotient rescale, fixed-point scale-aware half-even `round`, `reshape`, `broadcast`, single-output
...
scale-aware fixed-point `cast`/`round`, mixed-scale `add`/`sub`, mixed-scale `mul`, and `Fixed32` reciprocal `div` vectors using per-input and expected output dtype/scale metadata,
...
Remaining Tensor IR/conformance gaps: matmul accumulation/range policy,
low-level packed tensor storage/chunking APIs, index-consistency proofs for
