...
`reshape`, `broadcast`, `sum`, `reduce_sum`, `mean`, `clamp`, `squeeze`, `unsqueeze`, `slice`, `tril`, `triu`,
`concat`, `stack`, `matmul`, `full`, `arange`, and
...
multi-output expected tensors for exact quantize scale output, field-order comparison, selection, and clamp
