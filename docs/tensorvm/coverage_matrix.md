...
Public evidence CUDA gate note: `testnet::tests::public_testnet_evidence_bundle_requires_cuda_verified_miners_for_full_spec`
now proves that otherwise complete full-spec public evidence remains non-full-spec unless
`cuda_verified_miner_count` covers the counted public miners. The public evidence manifest parser requires
that field, and CLI validation reports both the count and boolean CUDA miner evidence gate.
