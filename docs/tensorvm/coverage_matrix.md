...
`gt`/`lt`/`ge`/`le`/`eq`, `where`, field-order `clamp`, `mean`, `cast`, `concat`, `stack`, `full`, and
`arange`). Tensor
...
`reshape`, `broadcast`, `sum`, `reduce_sum`, `mean`, `clamp`, `concat`, `stack`, `matmul`, `full`, `arange`, and
