...
`tril`, `triu`, `concat`, `stack`, `matmul`, `full`, `arange`, and
...
multi-output expected tensors for exact quantize scale output and dynamic-output `split`, field-order comparison, selection, and clamp
