...
- Active feature: Iteration 152 in progress - public drand freshness and chain-epoch mapping evidence.
- Current status: delayed proposer, receipt, challenge, validator-audit, and credit rewards are
  ...
  and chain-epoch evidence, and rejects locally fetched public rounds outside the configured lag.
  Production validator VRF signatures, consensus-level public drand epoch mapping, and deployed
...
- Next action: continue production VRF construction, consensus-level public drand epoch mapping, public/CUDA
...
| Randomness commit/reveal or VRF beacon | Partial | Receipt anchors bind finalized beacon randomness and validation seed commitments. Local runtimes ingest deterministic fixtures, configured verified drand, and public default-chain chained drand through verified chain commands. P2p/node paths relay bounded fixture, verified drand, public chained drand, and validator reveal payloads. Public drand polling exposes attempts/successes/stale/failure/backoff plus expected latest round, fetched lag, max lag, rounds per chain epoch, chain epoch, and freshness, and stale-by-policy local public rounds are skipped before chain mutation. Status/explorer/checker expose seed-domain, external beacon count/latest round, validator reveal count, role counters, network-applied beacon/reveal counters, and block-hash-ban evidence | Add consensus-level public drand epoch mapping, production validator VRF signatures, and deployed commit-reveal lifecycle |
...
Out of scope: consensus-level drand epoch mapping, public endpoint quorum/failover, production validator
VRF signatures, 7-day public deployment evidence, CUDA evidence, and full deployed commit-reveal lifecycle.
...
- Public 7-day evidence, CUDA evidence, production VRF keys, and full interactive transcript disputes remain
  deployment or future-feature gates, not local-completion claims.
