...
Latest attempted run: June 21, 2026 from the workspace root during Iteration 112 with:
...
This environment does not currently have `cargo-tarpaulin` installed, so Iteration 112 coverage could not
be regenerated. Iterations 78 through 112 rechecked the same command and hit the same missing-binary
