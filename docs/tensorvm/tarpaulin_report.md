...
Latest attempted run: June 21, 2026 from the workspace root during Iteration 113 with:
...
This environment does not currently have `cargo-tarpaulin` installed, so Iteration 113 coverage could not
be regenerated. Iterations 78 through 113 rechecked the same command and hit the same missing-binary
