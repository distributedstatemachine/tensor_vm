...
> division/comparisons/selection, bool masks, or the full CPU reference profile. The full frozen-registry CUDA vector suite and
...
> `mul`/`div`, exact field `identity`/`neg`/`abs`/`sign`, field comparison masks
...
- [~] Which Tier-B ops have *sound* random-linear checks vs. deterministic replay/fraud proofs: current
  ...
  selection. CUDA evidence now covers same-shape field modular division and field mask selection only;
