...
>
> Status (audit deterrent): because the agreement root is a deterministic function of the receipt's claimed outputs, a lazy validator could in principle commit it *without* re-executing; the honest-majority assumption is therefore enforced economically by the §12 mandatory randomized **validator audits**. Audit assignment is verification-class-agnostic (it samples every attestation, committee receipts included), the auditor re-verifies a Tier-C receipt through the committee verifier, and a `Valid` attestation contradicted by the auditor's canonical `Invalid` is **slashed** — proven end to end (a lazy committee validator who rubber-stamps a wrong Tier-C receipt loses stake). For committee receipts the audit also compares the **agreement root** (not just the pass/fail result), so a validator that attests `Valid` with a non-canonical root — lying about the result it re-executed, which would otherwise be invisible to a result-only audit and could poison the agreement count — is slashed even when the receipt is itself correct. Tier-A/B Freivalds roots are per-validator and are not compared.
>
> Status (runtime role pipeline): the production role functions handle a Tier-C committee receipt end to end — the **miner role** executes a committee graph via the canonical reference (`execute_graph_job_with_backend` routes `requires_committee_verification()` graphs through `committee_ir_execution_with_const_blobs`, closing the gap where the miner previously only ran the consensus path), and the **validator role** attests through the committee verifier with the deterministic agreement root. Proven by a role-function test (miner produces → validator verifies; strict path rejects). **Remaining for full readiness evidence:** a live multi-process run (the `deploy/tensorvm/local-cpu` Docker testnet) that emits Tier-C committee jobs from the synthetic producer, settles them across separate miner/validator node processes over libp2p, opens a Tier-C §8.2 dispute (requires a fault-injected wrong receipt), and a local-checker assertion of committee settlement + dispute outcomes — these are operator-run on the Docker box and are not exercised by `cargo test`. The detection-probability evidence (§12 calibration) no longer reports committee receipts under deterministic 100% `graph_exact_replay`; they appear under a `committee_agreement` mechanism whose enforceable detection is the validator-audit sample rate, and the `bond ≥ gain-from-fraud` invariant covers the committee validator reward via the `validator_audit` fraud path.
...
>
> Status (implemented — Tier-C disputes): the trace-bisection game (open → signed midpoint rounds → isolate one op → one-op referee verdict → slash loser + delayed challenger bounty, with responder/challenger timeouts) is implemented and **now resolves Tier-C committee receipts**. The one-op referee (`TensorGraph::referee_op` / `referee_witness`) and the runtime challenger's local replay (`execute_committee` / `validate_for_committee`) admit committee graphs, so a **single honest challenger** can punish a wrong Tier-C (e.g. `gelu`) receipt by isolating the disputed op and having the chain re-execute just that op against the canonical reference — 1-of-N honesty, independent of committee honesty. Trace openings are Merkle-only and were already admission-agnostic; the committee receipt's `trace_root` (from `execute_committee`) is bisected identically. Proven end to end: a fraudulent `gelu` receipt is refereed, the miner is slashed, and a delayed challenger bounty is recorded. **§8.1 → §8.2 escalation routing is wired:** when an assigned committee presents `≥ 2` distinct result roots (a genuine disagreement, vs. merely-insufficient attestations), `settle_epoch` records an explicit escalation (`reason: "tier-c committee disagreement escalated to fraud proof"`, with the conflicting-root count) and leaves the receipt unsettled, routing it to the §8.2 game. The chain deliberately does **not** auto-*open* the trace-bisection session: opening with bonds + responder timeouts would let anyone grief an honest miner (open, abandon, miner forfeits). Instead the game is opened by an incentivized 1-of-N honest challenger (the standard optimistic model), which is safe because an honest challenger only disputes a genuinely wrong receipt (the referee re-execution makes a frivolous challenge lose its bond). A single dishonest validator cannot trigger escalation, since the honest majority still reaches `k` on the canonical root and settles; escalation fires only when honest validators cannot reach `k`, which is exactly when §8.2's 1-of-N resolution is needed.
