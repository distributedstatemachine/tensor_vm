...
> bounds, fixed-point division/comparisons/selection/clamp, bool masks, or the full CPU reference profile. The full frozen-registry CUDA vector suite and
...
> `mul`/`div`, scalar-bounds field `clamp`, exact field `identity`/`neg`/`abs`/`sign`, field comparison masks
...
- [~] Which Tier-B ops have *sound* random-linear checks vs. deterministic replay/fraud proofs: current
  ...
  selection. CUDA evidence now covers same-shape field modular division, scalar-bounds field clamp, and field mask selection only;
