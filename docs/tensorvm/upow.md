...
> limited to the exercised same-shape field subset instead of over-claiming broadcasting, vector clamp
> bounds, fixed-point division/comparisons/selection/clamp, fixed-point or broadcast reductions, CUDA
> `mean`, bool masks, or the full CPU reference profile. The full frozen-registry CUDA vector suite and
...
> `mul`/`div`, scalar-bounds field `clamp`, deterministic field `sum`, exact field
> `identity`/`neg`/`abs`/`sign`, field comparison masks `eq`/`gt`/`lt`/`ge`/`le`, and mask-fed field
> `where` through miner-role `cuda:N` backend selection;
...
- [~] Which Tier-B ops have *sound* random-linear checks vs. deterministic replay/fraud proofs: current
  ...
  deterministic field sum, and field mask selection only;
