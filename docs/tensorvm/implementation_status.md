...
- Content-addressed Tensor IR foundation with typed tensor specs, params, refs, ops, graph outputs,
  ...
  CUDA graph ops, fixed-point `mean`, fixed-point or broadcast reductions, vector clamp bounds, bool-mask
  selection, and full frozen-registry CUDA graph execution remain open.
