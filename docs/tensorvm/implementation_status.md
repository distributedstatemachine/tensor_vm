...
- Content-addressed Tensor IR foundation with typed tensor specs, params, refs, ops, graph outputs,
  ...
  exact field modular-inverse and `Fixed32` reciprocal `div`, `scalar_mul`, `transpose`, explicit-dim
  ...
  (`matmul`/`add`/`sub`/`mul`/`div`/`clamp`/`sum`/`mean`/`reshape`/`broadcast`/`transpose`/`scalar_mul`/`relu`/`identity`/`neg`/`abs`/`sign`/`eq`/`gt`/`lt`/`ge`/`le`/`where`)
  through `GpuMinerBackend`; the full interactive dispute game, binary-op CUDA broadcasting, fixed-point
  CUDA graph ops, fixed-point `mean`/`reshape`, fixed-point or broadcast reductions,
  squeeze/unsqueeze/slice/split/concat/stack, vector clamp bounds, bool-mask selection, and full
...
- Optional `cuda-kernels` feature that builds `kernels/cuda/field_matmul.cu` with `nvcc`, routes the
  ...
  `add`/`sub`/`mul`/`div`/`clamp`/`relu`/`identity`/`neg`/`abs`/`sign`/`eq`/`gt`/`lt`/`ge`/`le`/`where`/`scalar_mul`/`transpose`
...
- `tvmd` binary tests for the documented spec-path pending manifest commands, proving
  ...
  manifest, reports `cuda_verified_miner_count=0`, reports `cuda_graph_execution_receipts=0`, reports
  `validator_vrf_lifecycle_records=0`, and reports `public_evidence_full_spec=false`
...
- Local CPU Compose gate: `docker compose -f deploy/tensorvm/local-cpu/docker-compose.yml up --wait`
  ...
  catch up to a passive observer finalized block-payload checkpoint after that target appears in p2p
  payload gossip with matching finalized block hash and state root via
  `all_operator_network_head_convergence=true`, plus p2p payload observation of that same network head hash,
...
- `cargo test -p tensor_vm --features cuda-kernels --release`: 580 TensorVM library tests and 54
  `tvmd_runtime` tests passed, including
  ...
  `runtime::tests::cpu_and_gpu_backends_match_supported_cuda_graph_ops`, and
  `miner_role::miner_role_submits_supported_multi_op_graph_execution_with_configured_cuda_backend`
...
- production GPU-miner packaging and a broader optimized CUDA/C++ kernel suite; the current native kernel
  ...
  `add`/`sub`/`mul`/`div`/`clamp`/`sum`/`mean`/`reshape`/`broadcast`/`relu`/`identity`/`neg`/`abs`/`sign`/`eq`/`gt`/`lt`/`ge`/`le`/`where`/`scalar_mul`/`transpose`,
...
- long-running public 7-day testnet with independent external operators; current implementation exposes
  ...
  observation records that aggregate to the network-runtime root, signed external artifact locators for raw supporting records, signed deployed detection-measurement records, deployed public-service
