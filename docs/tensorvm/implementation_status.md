...
- Content-addressed Tensor IR foundation with typed tensor specs, params, refs, ops, graph outputs,
  ...
  CUDA graph ops, fixed-point `mean`/`reshape`/`squeeze`/`unsqueeze`, fixed-point or broadcast reductions,
  slice/split/concat/stack, vector clamp bounds, bool-mask selection, and full
...
- production GPU-miner packaging and a broader optimized CUDA/C++ kernel suite; the current native kernel
  ...
  `add`/`sub`/`mul`/`div`/`clamp`/`sum`/`mean`/`reshape`/`squeeze`/`unsqueeze`/`broadcast`/`relu`/`identity`/`neg`/`abs`/`sign`/`eq`/`gt`/`lt`/`ge`/`le`/`where`/`scalar_mul`/`transpose`,
