...
- Content-addressed Tensor IR foundation with typed tensor specs, params, refs, ops, graph outputs,
  ...
  (`matmul`/`add`/`sub`/`mul`/`div`/`clamp`/`sum`/`mean`/`reshape`/`squeeze`/`unsqueeze`/`slice`/`broadcast`/`transpose`/`scalar_mul`/`relu`/`identity`/`neg`/`abs`/`sign`/`eq`/`gt`/`lt`/`ge`/`le`/`where`)
  through `GpuMinerBackend`; the full interactive dispute game, binary-op CUDA broadcasting, fixed-point
  CUDA graph ops, fixed-point `mean`/`reshape`/`squeeze`/`unsqueeze`/`slice`, fixed-point or broadcast reductions,
  split/concat/stack/tril/triu, vector clamp bounds, bool-mask selection, and full
...
- Optional `cuda-kernels` feature that builds `kernels/cuda/field_matmul.cu` with `nvcc`, routes the
  ...
  `add`/`sub`/`mul`/`div`/`clamp`/`sum`/`mean`/`reshape`/`squeeze`/`unsqueeze`/`slice`/`broadcast`/`relu`/`identity`/`neg`/`abs`/`sign`/`eq`/`gt`/`lt`/`ge`/`le`/`where`/`scalar_mul`/`transpose`
