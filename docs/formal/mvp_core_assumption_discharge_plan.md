...
| AD-015A | Verifier-dependent reward finality waits for challenges or direct recomputation. | `implementation-dischargeable`, `formalizable`, `evidence-dischargeable` | Local receipt, proposer, challenge, audit-related, and credit rewards now use pending claims, maturity release, voiding, and nonpayment/prune paths for implemented disputes. Networked full transcript disputes, direct recomputation evidence, public DA-through-window evidence, and theorem discharge remain incomplete. | Complete challenge openings tied to block `checks_root`, deterministic full-transcript dispute resolution, DA-through-window evidence, public propagation evidence, and settlement proofs. | "Local delayed reward ledgers are implemented for current paths; full reward finality remains open." |
